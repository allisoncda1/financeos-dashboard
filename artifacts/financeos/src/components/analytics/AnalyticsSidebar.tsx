...
const NAV_ITEMS = [
  { label: "Overview", icon: LayoutDashboard, href: "/analytics" },
  { label: "Cost Centers", icon: Network, href: "/analytics/cost-centers" },
  { label: "Shared Expenses", icon: Split, href: "/analytics/shared-expenses" },
  { label: "Allocation Rules", icon: ListChecks, href: "/analytics/allocation-rules" },
  { label: "Allocations", icon: BookOpen, href: "/analytics/allocations" },
  { label: "Department P&L", icon: Scale, href: "/analytics/department-pnl" },
  { label: "Entity Profitability", icon: Building2, href: "/analytics/entity-profitability" },
  { label: "Client Profitability", icon: Users, href: "/analytics/client-profitability" },
  { label: "Project Profitability", icon: FolderKanban, href: "/analytics/project-profitability" },
  { label: "Allocation Simulator", icon: FlaskConical, href: "/analytics/simulator" },
  { label: "Reports", icon: FileText, href: "/analytics/reports" },
  { label: "Settings", icon: Settings, href: "/analytics/settings" },
];

export function AnalyticsSidebar({ onClose }: { onClose?: () => void }) {
  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 px-2 overflow-y-auto pb-3 space-y-3 mt-2">
        <Section label="Analytics Module">
          {NAV_ITEMS.map((item) => {
            const isActive =
              item.href === "/analytics"
                ? pathname === "/analytics"
...
function NavItem({
  ...
  return (
    ...
    >
      {active && (
        <motion.span
          layoutId="nav-active-bg-analytics"
          className="absolute inset-0 rounded-lg"
          style={{ background: "rgba(99,102,241,0.14)", borderLeft: "2px solid #6366F1" }}
          transition={{ duration: 0.18, ease: [0.16, 1, 0.3, 1] }}
        />
      )}
      <span style={{ color: active ? "#A5B4FC" : "rgba(255,255,255,0.45)", display: "contents" }}>
