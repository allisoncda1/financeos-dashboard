...
- `cash_flow_gaap.csv`: a real QBO GAAP statement of cash flows exists per entity (`entity,report_date,activity_section,sub_section,label,amount,is_total`) — no need to derive cash flow from balance-sheet deltas. Section nets are the `is_total=True` "Net cash provided by …" rows; the "Other" section carries "Net cash increase for period" and "Cash at end of period". Drive also has `balance_sheet_prior_year.csv`, `pnl_prior_year.csv`, `invoice_lines.csv`, `payments_linked.csv`, `revenue_by_product.csv` (unused so far).
