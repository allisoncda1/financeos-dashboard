import { Document, Packer, Paragraph, Table, TableRow, TableCell, TextRun, AlignmentType, WidthType, BorderStyle, ShadingType, PageBreak, convertInchesToTwip, UnderlineType } from "docx";
...
function kpiRow(label: string, value: string, sub?: string): Paragraph {
  const runs: TextRun[] = [
    ...
    new TextRun({ text: value, font: FONT, size: 22, bold: true, color: NAVY }),
  ];
...
export const WordRenderer: Renderer = {
  ...
  async render(report: BuiltReport): Promise<Buffer> {
    ...
    // ── Cover page ──
    els.push(
      new Paragraph({ children: [new TextRun({ text: "", size: 24, font: FONT })], spacing: { before: convertInchesToTwip(0.8) } }),
      new Paragraph({
        children: [new TextRun({ text: entityName, font: FONT, bold: true, size: 64, color: NAVY })],
        spacing: { after: 160 },
      }),
      new Paragraph({
        children: [new TextRun({ text: report.template.name, font: FONT, size: 36, color: ACCENT, bold: true })],
        spacing: { after: 120 },
      }),
      new Paragraph({
        children: [new TextRun({ text: report.period, font: FONT, size: 28, color: GRAY })],
        spacing: { after: 80 },
      }),
      new Paragraph({
        children: [new TextRun({ text: "Generated " + new Date(report.generatedAt).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" }), font: FONT, size: 20, color: GRAY })],
        spacing: { after: convertInchesToTwip(0.4) },
      }),
      new Paragraph({
        border: { top: { style: BorderStyle.SINGLE, size: 6, color: ACCENT } },
        children: [new TextRun({ text: "Prepared from QuickBooks Online records. Internal management reporting, not an audited statement.", font: FONT, size: 16, color: GRAY, italics: true })],
    ...
    if (last12.length > 0) {
      ...
      const plRows = last12.map((m, i) => {
        ...
        return tRow([fmtMonth(m.month), fmtUsd(m.revenue), fmtUsd(m.gross_profit), fmtPct(gm), fmtUsd(m.net_income), fmtPct(nm)], colW, i % 2 === 1, isCur);
      });
      els.push(
    ...
    if (aging.length > 0) {
      ...
      els.push(
        new Table({
          ...
          rows: [
            tHeader(["Aging Bucket", "Amount"], colW),
            ...aging.map((r, i) => tRow([r.label, fmtUsd(r.amount)], colW, i % 2 === 1)),
            tTotalRow(["Total AR", fmtUsd(total)], colW),
          ],
          borders: noBorder(),
        }),
        new Paragraph({ text: "", spacing: { after: 120 } }),
    ...
    els.push(
      new Paragraph({ children: [new PageBreak()] }),
      new Paragraph({
        children: [new TextRun({ text: "Disclaimer", font: FONT, bold: true, size: 18, color: GRAY })],
        spacing: { after: 80 },
      }),
      new Paragraph({
        children: [new TextRun({ text: "This report was prepared from QuickBooks Online records and is intended for internal management use only. It does not constitute an audited financial statement.", font: FONT, size: 16, color: GRAY, italics: true })],
        spacing: { after: 80 },
      }),
      new Paragraph({
        children: [new TextRun({ text: "FinanceOS Report Engine  |  " + entityName + "  |  " + report.period, font: FONT, size: 16, color: GRAY })],
      }),
    );
    ...
    const doc = new Document({
      styles: {
        default: {
          document: {
            run: { font: FONT, size: 20, color: NAVY },
          },
        },
      },
      sections: [{
