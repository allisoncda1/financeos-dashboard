...
import { renderMonthlyClose } from "./src/reports/renderers/monthlyClose.js";
import { renderQuarterlyClose } from "./src/reports/renderers/quarterlyClose.js";
import { renderBoardPackage } from "./src/reports/renderers/boardPackage.js";
import { renderInvestorUpdate } from "./src/reports/renderers/investorUpdate.js";
import { renderBankPackage } from "./src/reports/renderers/bankPackage.js";
import { renderExecutivePackage } from "./src/reports/renderers/executivePackage.js";
import type { BuiltReport } from "./src/reports/builder.js";
import type { ReportTemplate } from "./src/reports/templates.js";
...
const smileMoreReport: BuiltReport = {
  ...
  branding: {
    mode: "single",
    primaryEntity: { slug: "Smile_More", name: "Smile More", logoPath: "/logos/smile-more-dark.png", primaryColor: "#ec4899" },
    entities: [{ slug: "Smile_More", name: "Smile More", logoPath: "/logos/smile-more-dark.png" }],
    financeosBranding: false,
  },
  generatedAt: new Date().toISOString(),
  period: "July 2026",
...
const multiReport: BuiltReport = {
  ...
  branding: {
    ...
    entities: [
      ...
      { slug: "Smile_More",   name: "Smile More",   logoPath: "/logos/smile-more-dark.png" },
    ],
...
// ─── T3 Marketing single-entity report (Task 7 metrics) ───────────────────────

const t3SingleMonthlyPL = [
...
function t3SingleMetrics() {
  return {
    ...
    open_ar: 150_000, open_ap: 45_000,
    dso_days: 38, dso_days_standard: 32, weighted_average_days_overdue: 5, dpo_days: 22,
    cash_on_hand: 210_000, ar_overdue_pct: 30.0, ap_overdue_pct: 1.5,
  };
}

function buildT3SingleFinancials() {
  return {
    entity_slug: "T3_Marketing",
    as_of: "2026-07-16",
    monthly_pl: t3SingleMonthlyPL,
    ytd_summary: { revenue: 890_000, cogs: 356_000, gross_profit: 534_000, opex: 267_000, net_income: 267_000 },
    balance_sheet: {
      as_of: "2026-07-16",
      prior_as_of: "2026-06-30",
      assets: {
        cash: 210_000, cash_prior: 175_000,
        accounts_receivable: 150_000, accounts_receivable_prior: 130_000,
        prepaid_expenses: 25_000, prepaid_expenses_prior: 22_000,
        equipment_net: 100_000, equipment_net_prior: 105_000,
        total: 485_000, total_prior: 432_000,
      },
      liabilities: {
        accounts_payable: 45_000, accounts_payable_prior: 40_000,
        accrued_liabilities: 20_000, accrued_liabilities_prior: 18_000,
        deferred_revenue: 10_000, deferred_revenue_prior: 8_000,
        notes_payable: 80_000, notes_payable_prior: 80_000,
        total: 155_000, total_prior: 146_000,
      },
      equity: {
        paid_in_capital: 130_000, paid_in_capital_prior: 130_000,
        retained_earnings: 200_000, retained_earnings_prior: 156_000,
        total: 330_000, total_prior: 286_000,
      },
    },
    cash_flow: {
      as_of: "2026-07-16",
      sections: [
        {
          ...
          net_cash: 97_500,
          lines: [
            { label: "Beginning Cash Balance", amount: 175_000, is_subtotal: false },
            { label: "Net Income", amount: 97_500, is_subtotal: false },
          ],
        },
        { name: "Investing Activities", net_cash: -5_000, lines: [{ label: "Equipment", amount: -5_000, is_subtotal: false }] },
        { name: "Financing Activities", net_cash: 0, lines: [] },
      ],
      net_cash_change: 35_000,
      cash_at_end: 210_000,
    },
    cash_history: [
      { label: "May", value: 145_000 },
      { label: "Jun", value: 175_000 },
      { label: "Jul", value: 210_000 },
    ],
  };
}

const t3SingleAlerts = [
  {
    entity: "T3 Marketing",
    title: "AR Overdue — 30 Percent",
    description: "30 percent of accounts receivable ($45,000) is overdue. The rate has increased from the prior period average.",
    severity: "medium",
    recommended_action: "Send payment reminders to overdue accounts. Prioritize accounts over 45 days for direct outreach.",
    financial_impact: "$45,000 at elevated collection risk",
  },
];

const t3SingleReport: BuiltReport = {
  id: "t3-marketing-single",
  template: TPL,
  request: { template: "monthly-close", entities: ["T3_Marketing"], period: "July 2026", format: "pdf" },
  branding: {
    mode: "single",
    primaryEntity: { slug: "T3_Marketing", name: "T3 Marketing", logoPath: "/logos/t3-marketing.png", primaryColor: "#f59e0b" },
  ...
  sections: {
    executive_summary: {},
    portfolio_kpis: { portfolio: null },
    entity_summary: { T3_Marketing: { metrics: t3SingleMetrics(), anomalies: [] } },
    financials: { T3_Marketing: buildT3SingleFinancials() },
    ar_ap: {},
    alerts: t3SingleAlerts,
  ...
  metadata: { entityCount: 1, dataFreshness: "2026-07-16", confidenceScore: 0.96 },
};

// ─── Multi-template report fixtures ───────────────────────────────────────────

function makeTemplateReport(id: string, name: string, templateId: string): BuiltReport {
  return {
    ...multiReport,
    id,
    template: { ...TPL, id: templateId, name },
  };
}
...
async function generateReport(report: BuiltReport, slug: string, renderFn: (r: BuiltReport) => string = renderMonthlyClose) {
  const html = renderFn(report);
...
async function main() {
  ...
  // Existing monthly-close reports
  ...
  // New: T3 Marketing single-entity monthly close
  await generateReport(t3SingleReport, "monthly-close-t3-marketing-single", renderMonthlyClose);

  // New: All other template types using the portfolio fixture
  await generateReport(makeTemplateReport("quarterly-close-portfolio", "Quarterly Close Report", "quarterly-close"), "quarterly-close-portfolio-all", renderQuarterlyClose);
  await generateReport(makeTemplateReport("board-package-portfolio", "Board Package", "board-package"),             "board-package-portfolio-all", renderBoardPackage);
  await generateReport(makeTemplateReport("investor-update-portfolio", "Investor Update", "investor-update"),       "investor-update-portfolio-all", renderInvestorUpdate);
  await generateReport(makeTemplateReport("bank-package-portfolio", "Bank Package", "bank-package"),               "bank-package-portfolio-all", renderBankPackage);
  await generateReport(makeTemplateReport("executive-package-portfolio", "Executive Package", "executive-package"), "executive-package-portfolio-all", renderExecutivePackage);
