...
export default function CategorizedTransactionsPage() {
  ...
  const categorized = bank?.transactions.filter((t) => !!t.category) ?? [];
  const uncatCount  = (bank?.transactions.length ?? 0) - categorized.length;
  ...
  const SUB_NAV = [
    ...
    { label: "Categorized",   href: "/accounting/transactions/categorized",   count: categorized.length },
  ];
      ...
      <div className="bg-white border-b border-gray-100 px-6 py-4 flex-shrink-0">
        <div className="flex items-center justify-between gap-4 mb-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-[16px] font-bold text-gray-900 leading-tight">Categorized Transactions</h1>
              <p className="text-[11px] text-gray-400">
                {bank ? `${cfg.name} · ${categorized.length} categorized · As of ${bank.as_of}` : "Select an entity"}
              </p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
              <h3 className="text-[13px] font-semibold text-gray-900">Categorized Transactions</h3>
              <span className="text-[10px] text-gray-400">{categorized.length} shown</span>
            </div>
            <TransactionTable
              transactions={categorized}
              emptyLabel="No categorized transactions found."
            />
          </div>
        </div>
      )}
